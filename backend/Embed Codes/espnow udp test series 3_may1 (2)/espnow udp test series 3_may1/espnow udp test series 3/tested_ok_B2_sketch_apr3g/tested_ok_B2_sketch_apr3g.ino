...
String stationID = "B/2"; // Default, will be loaded from EEPROM
...
User users[MAX_USERS] = {
  {{0xE2, 0xA0, 0x42, 0x2}, "Swarajit", "B01", true},
  {{0x23, 0x48, 0xC9, 0x2C}, "Apratim", "B02", true}
};
...
typedef struct struct_message {
    ...
    char stnID[10];       // Station ID (e.g., "B/2") from EEPROM
...
// Note: Saving individual config items might be better done in their respective setting functions
// This is a placeholder for potentially saving all at once if needed.
void saveConfigToEEPROM() {
...
// Unified function to prepare and send data via the selected protocol
void sendAlertData(bool verified) {
...
// Display communication status on OLED and Serial
...
// Update sensor readings (OLD LOGIC BASE)
...
// Function 1: automode (From FIRST code)
void automode() {
...
// Function 2: isRealAlert (From FIRST code)
...
// Function 3: calculateSeverity (From FIRST code)
...
// Function 4: handleFalsePositives (From FIRST code)
...
// Called when touch sensor held long enough
void activateManualMode() {
...
// Handles keypad input during different stages of manual mode
void handleManualMode() {
...
// Reset state variables and display after manual mode ends or is cancelled
void resetManualMode() {
...
// Entry point for the main menu, triggered by touch sensor
void showMainMenu() {
...
// Display the main menu options on the OLED
void displayMenu() {
    display.clearDisplay();
    display.setTextSize(1);
    display.setCursor(0, 0);
...
// Handle switching communication mode
void switchCommMode() {
...
// Main handler for the User Management sub-menu
void handleUserManagement() {
...
// Add a new user (Guest) - Requires current user to be OG
void addUser() {
    if (!currentUserIsOG) {
...
// Remove the currently authenticated admin user's card
void removeSelf() {
...
// Remove a Guest user's card (requires current user to be OG)
void removeGuest() {
...
// Promote a Guest user to OG status (requires current user to be OG)
void promoteUser() {
     if (!currentUserIsOG) return;
...
void loadUsersFromEEPROM() {
    ...
    if (userCount < OG_USERS || userCount > MAX_USERS) {
        ...
        userCount = OG_USERS;
        users[0] = {{0xE2, 0xA0, 0x42, 0x2}, "Swarajit", "B01", true};
        users[1] = {{0x23, 0x48, 0xC9, 0x2C}, "Apratim", "B02", true};
...
// Save the current user array and count to EEPROM
void saveUsersToEEPROM() {
...
// Check for RFID tag, validate UID, and find user details (for Manual Mode activation)
// Returns true if a VALID, REGISTERED tag is found, false otherwise.
// Updates global currentUser, currentUserID, currentUserIsOG, currentUID on success.
...
// Basic validation for a 4-byte UID
bool validateUID(uint8_t* uid) {
...
// Find the index of a user in the 'users' array by their 4-byte UID
int findUserIndexByUID(const uint8_t* uid_to_find) {
...
// Reset display to the default idle/auto mode screen
void resetDisplay() {
    display.clearDisplay();
    display.setTextSize(1);
    display.setTextColor(SH110X_WHITE);
...
// Display prompt for selecting fire type
void displayFireTypePrompt() {
    display.clearDisplay();
    display.setCursor(0, 0);
...
// Display prompt for selecting fire intensity
void displayFireIntensityPrompt() {
    display.clearDisplay();
    display.setCursor(0, 0);
    display.println("--- Manual Alert ---");
...
// Display prompt for verification (* to send, # to cancel)
void displayVerificationPrompt() {
    display.clearDisplay();
    display.setCursor(0, 0);
...
// Show an invalid input message temporarily
void displayInvalidInput(String message) {
    display.clearDisplay();
    display.setCursor(0, 10);
...
// Show the selected fire type temporarily
void displaySelectedFireType() {
    display.clearDisplay();
    display.setCursor(0, 10);
...
// Show the selected fire intensity temporarily
void displaySelectedFireIntensity() {
    display.clearDisplay();
    display.setCursor(0, 10);
...
// Update global GPS variables by reading from SoftwareSerial
void updateGPSData() {
...
// Convert decimal degrees GPS coordinate to Degrees Decimal Minutes (DDM) string
String convertToDDM(double decimalDegrees, bool isLatitude) {
...
// Check if keypad/menu entry has timed out
...
// Reset the keypad/menu timeout timer (e.g., on entering menu or pressing a key)
void resetKeypadTimeout() {
