...
void loop() {
  ...
  if (currentPump.isActive) {
    ...
    if (!autoOverrideActive) {
      if (abs(pot - lastPotValue) > POT_DEADZONE) {
        autoOverrideActive = true;
      } else {
        // use automated distance-based speed
        analogWrite(MOTOR_PWM_PIN, pwmOut(currentPump.initialSpeed));
      }
    }
    if (autoOverrideActive) {
      // now take operator control
      int pwm = pot / 4;
      analogWrite(MOTOR_PWM_PIN, pwmOut(pwm));
    }
    lastPotValue = pot;
    ...
    if (millis() - lastUpdate >= 200) {
      updateFiringScreen(autoOverrideActive ? (pot/4) : currentPump.initialSpeed, remaining);
      lastUpdate = millis();
    }
    ...
    // Read potentiometer for manual mode display
