/*
 * Fire-suppression turret controller with Joystick
 ...
 *  – Joystick control: VRY-A1 (Y-axis), VRX-A0 (X-axis)
 */
...
// Joystick pins
#define JOY_X_PIN A0  // VRX
#define JOY_Y_PIN A1  // VRY
...
// Joystick variables
const int JOY_DEADZONE = 50;  // Deadzone to prevent drift
unsigned long lastJoystickMove = 0;
const unsigned long JOYSTICK_DELAY = 5000; // Return to normal mode after 5s of inactivity
...
void handleJoystick() {
  ...
  bool xActive = (joyXValue < (512 - JOY_DEADZONE)) || (joyXValue > (512 + JOY_DEADZONE));
  bool yActive = (joyYValue < (512 - JOY_DEADZONE)) || (joyYValue > (512 + JOY_DEADZONE));
  
  if (!xActive && !yActive) {
    if (joyActive) {
      // Joystick was active but now returned to center
      joyActive = false;
      lastJoystickMove = millis();
    }
    return;  // No joystick input
  }
  
  // If we're entering joystick mode from other modes
  if (!joyActive) {
    joyActive = true;
    modeX = 'J';  // Special joystick mode
    modeY = 'J';
  }
  
  // Handle X-axis movement only if active
  if (xActive) {
    if (joyXValue < (512 - JOY_DEADZONE)) {
      // Move left (negative direction)
      digitalWrite(DIR_PIN, LOW);
      stepPulse(STEP_PIN);
      currentAngleX -= 0.1;  // Small angle increment
      if (currentAngleX < 0) currentAngleX += 360;
    } 
    else if (joyXValue > (512 + JOY_DEADZONE)) {
  ...
  // Handle Y-axis movement only if active
  if (yActive) {
    if (joyYValue < (512 - JOY_DEADZONE)) {
      // Move up (positive direction)
      if (currentAngleY < 75) {
        digitalWrite(Y_DIR_PIN, HIGH);
        stepPulse(Y_STEP_PIN);
        currentAngleY += 0.1;  // Small angle increment
      }
    } 
    else if (joyYValue > (512 + JOY_DEADZONE)) {
...
void setup()
{
  ...
  Serial.println(F("Joystick active: VRX-A0 (X-axis), VRY-A1 (Y-axis)"));
}
...
void loop()
{
  ...
  // Return to normal mode if joystick inactive for 5 seconds
  if ((modeX == 'J' || modeY == 'J') && 
      (millis() - lastJoystickMove > JOYSTICK_DELAY)) {
    modeX = 'X';
    modeY = 'X';
  }

  // ░░░░░ Mode handlers ░░░░░
  // X (only if not in joystick mode)
  if (modeX == 'C' && modeX != 'J')       stepPulse(STEP_PIN);
  else if (modeX == 'A' && modeX != 'J') { angleMoveX(90.0); modeX = 'X'; }
  else if (modeX == 'R' && modeX != 'J') { angleMoveX(-currentAngleX); currentAngleX = 0; modeX = 'X'; }

  // Y (only if not in joystick mode)
  if (modeY == 'C' && modeY != 'J')       stepPulse(Y_STEP_PIN);
  else if (modeY == 'A' && modeY != 'J') { rotateYToAngle(45.0); modeY = 'X'; }
  else if (modeY == 'R' && modeY != 'J') { rotateYToAngle(0.0);  modeY = 'X'; }

  // ░░░░░ Idle auto-home after 60 s ░░░░░
  if (modeX == 'X' && modeX != 'J' && millis() - lastModeChange > 60000 && fabs(currentAngleX) > 0.1) {
    angleMoveX(-currentAngleX); currentAngleX = 0.0;
  }
  if (modeY == 'X' && modeY != 'J' && millis() - lastModeChange > 60000 && fabs(currentAngleY) > 0.1) {
    rotateYToAngle(0.0);
  }
}